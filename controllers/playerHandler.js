...
let playerHandler = async (player, fields) => {
  return  {
    findAll: async (req, res) => {
      try {
        let all = await player.findAll({order: ['id']})
        if(!all) 
          res.status(404).send({
            status: 'error',
    ...
    findOne: async (req, res) => {
      try {
        let p = await player.findById(req.params.id)
        if(!p)
          res.status(404).send({
            status: 'error',
            message: `Route ${req.url} for method ${req.method} not found`,
    ...
    insert: async (req, res) => {
      try {
        ...
        if(!emptyFields.length)
          res.send(await player.create(req.body))
        else
          res.status(400).send({
            status: 'error',
